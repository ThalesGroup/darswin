...
from .Woodscapes import Woodscape
...
def build_loader(config,xi=None):
    config.defrost()
...
def build_dataset(is_train, config,xi=None):
    ...
    if config.DATA.DATASET == 'imagenet':
        ...
        if is_train:
            ...
            dataset = Woodscape(config.DATA.DATA_PATH, config.MODEL.DISTORTION, config.DATA.low, config.DATA.high, task = 'val', transform = transform)
            nb_classes = 4
...
def build_transform(is_train, config):
    ...
    if is_train:
        ...
        transform_primary, transform_secondary, transform_final = create_transform(
        ...
        transform = transforms.Compose(transform_secondary + transform_final)
