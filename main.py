...
def parse_option():
    ...
    parser.add_argument('--xi_bool', type=bool, default=True)
...
def main(config):
    ...
    if config.MODEL.RESUME:
        ...
        logger.info(f"Top 5 Accuracy of the network on the {len(dataset_val)} test images: {acc5:.1f}%")
        infile= open("Results_" + config.MODEL.NAME + ".csv", "a")
        writer = csv.writer(infile)
        res=[config.MODEL.XI, acc1, acc5]
        writer.writerow(res)
        if config.EVAL_MODE:
            return
...
if __name__ == '__main__':
    ...
    if not os.path.exists("Results_" + config.MODEL.NAME + ".csv"):
        headerList=['xi', 'acc1', 'acc5']
        with open("Results_" + config.MODEL.NAME + ".csv", 'a+') as file:
